def handle_request(server, request, conn):
    ...
    print(request)
    for i in range(
            len(value)):  # for each function that represent the value of the first part of the request (before the first slash, all the function along with the data as a parameter)
        func = value[i]
        try:
